  ...
  return (
    <div className='relative space-y-1.5'>
      <div className='relative'>
        <InputGroup>
          <Input
            ...
            className={clx('rounded-r-none', className)}
      ...
      <React.Activity mode={strengthIndicator ? 'visible' : 'hidden'}>
        <PasswordStrength value={currentValue} config={passwordStrengthConfig} className='mt-4' />
      </React.Activity>
    </div>
  )
}
...
export function PasswordStrength({ value, className, config }: PasswordStrengthProps) {
  ...
  return (
    <div className={clx('px-0.5', className)}>
      <div className='mb-1 flex items-center justify-between'>
        <span className='text-muted text-sm'>Password strength</span>
        <span className='text-muted text-sm'>{passwordValue ? label : ''}</span>
      </div>
      ...
      {requirements.length > 0 && passwordValue.length > 0 && (
        <ul className='mt-2.5 space-y-1.5 text-sm text-gray-500'>
