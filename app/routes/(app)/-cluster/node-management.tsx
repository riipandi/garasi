...
export function NodeManagement({ nodeId, nodes: nodeIds }: NodeManagementProps) {
  const queryClient = useQueryClient()
  const [selectedNode, setSelectedNode] = React.useState<string | undefined>(nodeId)
  ...
  const { data: nodeInfoData } = useQuery({
    queryKey: ['cluster', 'node', 'info', selectedNode],
    queryFn: () =>
  ...
  const nodeInfo = nodeInfoData?.success
  const nodes = nodeIds || Object.keys(nodeInfo || {})
  ...
  const handleRepair = () => {
    if (selectedNode) {
      repairMutation.mutate({ node: selectedNode, repairType: selectedRepairType })
    }
  }
  ...
  return (
    <div className='rounded-lg border border-gray-200 bg-white p-4 shadow-sm sm:p-6'>
      <div className='mb-4 flex items-center justify-between'>
        <h2 className='text-lg font-semibold sm:text-xl'>Node Management</h2>
        <Lucide.Settings2 className='size-5 text-gray-400' />
      </div>

      {/* Node Selection */}
      <div className='mb-4'>
        <label htmlFor='node-select' className='mb-2 block text-sm font-medium text-gray-700'>
          Select Node
        </label>
        <select
          id='node-select'
        ...
        <div className='mb-4 rounded-lg border border-gray-200 bg-gray-50 p-4'>
          ...
          <div className='grid grid-cols-1 gap-3 sm:grid-cols-2'>
            <div>
              ...
              <p className='text-sm font-medium text-gray-900'>{nodeInfo[selectedNode].nodeId}</p>
            </div>
            <div>
              <p className='text-xs text-gray-500'>Garage Version</p>
              <p className='text-sm font-medium text-gray-900'>
                {nodeInfo[selectedNode].garageVersion}
              </p>
            </div>
            <div>
              <p className='text-xs text-gray-500'>Rust Version</p>
              <p className='text-sm font-medium text-gray-900'>
                {nodeInfo[selectedNode].rustVersion}
              </p>
            </div>
            <div>
              <p className='text-xs text-gray-500'>Database Engine</p>
              <p className='text-sm font-medium text-gray-900'>{nodeInfo[selectedNode].dbEngine}</p>
            </div>
          </div>

          {nodeInfo[selectedNode].garageFeatures && (
            <div className='mt-3'>
              <p className='mb-1 text-xs text-gray-500'>Garage Features</p>
              <div className='flex flex-wrap gap-1'>
                {nodeInfo[selectedNode].garageFeatures?.map((feature, index) => (
                  <span
                    key={index}
      ...
      {!selectedNode && (
        <div className='rounded border border-gray-200 bg-gray-50 px-3 py-8 text-center text-sm text-gray-700'>
          ...
          <p>Select a node to view details and perform operations</p>
        </div>
      )}
    </div>
  )
