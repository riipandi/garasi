...
export default defineProtectedHandler(async (event) => {
  ...
  const log = logger.withPrefix('AllowBucketKey')
  ...
  if (!body?.permissions) {
    log.warn('Permissions are required')
    throw new HTTPError({ status: 400, statusText: 'Permissions are required' })
  }

  log.withMetadata({ bucketId: id, accessKeyId: body.accessKeyId }).debug('Allowing bucket key')
